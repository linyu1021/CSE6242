...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        for i in range(n):
            j = random.choice(XX)
            samples.append(j[0:len(j)-1])
            labels.append(j[len(j)-1])
    ...
    def fitting(self):
        ...
        for i in range(self.num_trees):
           samples, labels = self.bootstraps_datasets[i], self.bootstraps_labels[i]
           self.decision_trees[i].learn(samples, labels)      
