...
def entropy(class_y):
    ...
    entropy = 0
    numEntries = len(class_y)
    if numEntries <= 1:
        return 0
    numCounts = np.bincount(class_y)
    prob = numCounts / float(numEntries)
    for i in prob:   
...
def partition_classes(X, y, split_attribute, split_val):
    ...
    if(isinstance(split_val, str)):
        for i in range(len(X[: ,split_attribute])):
...
def information_gain(previous_y, current_y):
    ...
    numEntries_1 = len(current_y[0])
    numEntries_2 = len(current_y[1])
    Entries_total = numEntries_1 + numEntries_2
    prob_left = float(numEntries_1) / float(Entries_total)
    prob_right = float(numEntries_2) / float(Entries_total)

    info_gain = H - prob_left * H_left - prob_right * H_right
